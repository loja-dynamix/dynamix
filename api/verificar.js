...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // CORREÇÃO: Usar searchRes aqui, não search
    ...
    // Se não achou na busca, tenta pelo ID direto (ID da preferência/checkout)
    const resId = await fetch(`https://api.mercadopago.com/v1/payments/${id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    const dataId = await resId.json();
    return res.status(200).json({ status: dataId.status || 'pending' });
